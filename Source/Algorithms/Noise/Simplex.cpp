...
inline static std::array<std::int16_t, 256> p{ 151, 160, 137, 91, 90, 15,
...
Algorithms::Simplex::Simplex() noexcept
{
	grad3[0] = { 1, 1, 0 };
	grad3[1] = { -1, 1, 0 };
	grad3[2] = { 1, -1, 0 };
	...
	{
		perm[i] = p[i & 255];
		permMod12[i] = static_cast<std::int16_t>(perm[i] % 12);
	}
}

double Algorithms::Simplex::dot(const Algorithms::Simplex::Grad& g, double x, double y)
{
	return g.x * x + g.y * y;
}

float Algorithms::Simplex::noise(const std::array<double, 2>& input)
{
